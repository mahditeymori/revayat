...
//
// Money is integer Rial everywhere, matching the rest of the codebase - Zibal's
// `amount` is also Rial, so no conversion happens.
import 'server-only';
import { SANDBOX_MERCHANT, isConfigResult, isPayableAmount, resultMessage } from './codes';

// The code tables and the pure verify decision live in codes.ts so they stay
// testable without a request scope; re-exported here so callers need only
// this module.
export * from './codes';
...
export type ZibalCall<T> = { ok: true; data: T } | { ok: false; result: number | null; message: string };
  ...
  try {
    ...
    console.error(
      `[zibal] CONFIGURATION ERROR: ZIBAL_MERCHANT is not set, so ${path} was never called. ` +
        'No customer can pay until it is set in the server environment and the container is recreated.',
    );
 ...
 * anything else is a configuration or amount problem on our side. Guarantees
 * it never returns ok:true without a trackId, so callers only need to check
 * `!call.ok` to know whether they have a usable session.
 */
export async function requestPayment(input: ZibalRequestInput): Promise<ZibalCall<ZibalRequestResponse>> {
 ...
 */
export async function verifyPayment(trackId: string | number): Promise<ZibalCall<ZibalVerifyResponse>> {
 ...
 */
export async function inquirePayment(trackId: string | number): Promise<ZibalCall<ZibalInquiryResponse>> {
