...
export default async function CollectionsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-3">
          {categories.map((c) => (
            <Link
              ...
              className="group relative block aspect-[4/3] overflow-hidden bg-cream-200"
            >
              {c.image && (
                <Image
                  src={c.image.url}
              ...
              <div className="absolute inset-x-0 bottom-0 p-5">
                ...
                <p className="mt-1 text-xs text-cream/75">{c.productCount} محصول</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      {products.length === 0 ? (
        <p className="mt-8 py-16 text-center text-sm text-ink-60">هنوز محصولی ثبت نشده است.</p>
      ) : (
